...
- 🔴 **Solo envía SALIENTES, no ENTRADA** ("no sabe cómo se descargaba el otro") → **las llamadas de entrada de agosto se están perdiendo** (no llegan a SQL ni al dashboard). **NO se capacita a Steven** (es Supervisor Contact Center, NO el reemplazo de Sergio → no se le cargan las tareas de datos). **Mientras tanto él no puede enviar la base de datos completa** (solo salientes), así que el hueco de entrada **persiste** hasta que llegue el **reemplazo real de Sergio** o se **automatice la extracción de ContactVox** (Airflow).
